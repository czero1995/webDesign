...
//        die;
    ...
    public function get_stadium_recommend($params)
    {
        ...
        $res = $this->field('id,title,address,post,booking_url')->order('is_recommend asc, id desc')->limit(($offset-1) * $page,$page)->select();
    ...
    public function get_stadium_sort($params)
    {
        ...
        $res = $this->field('id,title,address,post,booking_url')->order('sort desc, id desc')->limit(($offset-1) * $page,$page)->select();
        return ['data'=>$res, 'page'=>$offset];
    }
...
//        $res = Db::query($sql);
        ...
        $res = $this->field('id,title,address,post,booking_url')
                    ->where($wherestr1)
                    ->where($wherestr2)
