...
//                }
                ...
                $sql = 'select b.* from stadium_course as a left join course as b on b.id = a.course_id where a.stadium_id = ?';
                $course = Db::query($sql, [$val['id']]);

                $data[$key]['course_info'] = $course ; //课程信息
                $data[$key]['count_course'] =   count($course); //课程数
            }
