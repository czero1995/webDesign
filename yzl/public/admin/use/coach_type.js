    ...
    columns :
        [
            ...
            },
            {
                ...
                {
                    var edit = '<button class="btn btn-primary btn-xs edit_coach_type" id='+row.id+' data-toggle="modal" data-target="#edit_coach_type" data-backdrop="static"><i class="fa fa-edit"></i> 修改</button>'
...
//表单清空
$('#add_coach_type').on('hide.bs.modal', function () {
    document.getElementById("form1").reset();
})

//添加教练类型
$('#coach_type_add').click(function(){

    $(this).attr('disabled',true);

    $.ajax({
        url: admin_url+'Coachtype/add',
        type: 'post',
        data: {
        ...
        success: function(res) {
            ...
            if(res.errcode == 200) {
                layer.msg('添加成功',{icon:6,title:'温馨提示'},function(){
                    window.location =admin_url+"coachtype/index";
                });
        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.errcode == 200) {
                    ...
                    /*$('#type_table').bootstrapTable('refresh',{url: '/admin/admin/getCoachType'});*/
                } else {
...
$('#btn_delete').click(function(){
    ...
    layer.confirm('确定要删除所选项吗', {
        ...
        $.ajax({
            url: admin_url+'admin/delCoachType',
            type: 'post',
            data: {'id':arrid},
            success: function(res) {
                if(res.errcode == 200) {
...
//表单清空
$('#edit_coach_type').on('hide.bs.modal', function () {
    $('.einput').removeAttr("checked"); //移除属性防止干扰
})

//修改教练页面
$('body').on('click','.edit_coach_type',function(){
    var _this = $(this);
    $.ajax({
        url: admin_url+'coachtype/edit',
        type: 'post',
        data: {
...
//修改教练动作
$('#coach_type_edit').click(function(){

    $(this).attr('disabled',true);

    $.ajax({
        url: api_url+'coachcategory/editCoachCate',
        type: 'post',
        data: {
            'id': $('#eid').val(),
        ...
        success: function(res) {
            ...
            if(res.status == 200 || res.errcode == 200) {
                ...
                /*$('#edit_coach_type').modal('hide');
                $('#type_table').bootstrapTable('refresh',{url: '/admin/admin/getCoachType'});

                layer.msg('修改成功',{icon:6,title:'温馨提示'});*/
