...
class Coach extends Base
{
    ...
    public function get_coach_recommend($params)
    {
        ...
        //教练列表
        $coach_list =  $this->where('status', 1)
            ->where($map)
            ->order('id', 'desc')
...
//                    ->select();
        $sql = " select id, name, headimg, intro from coach where status = ? order by sort desc $limit";
        $res = Db::query($sql, [1]);
    ...
    public function get_coach_screen($where)
    {
        ...
        try {
            ...
            $sql = " select a.id, a.name, a.headimg, a.intro
                     from coach a
                     where ($wherestr1) and ($wherestr2) and ($wherestr3) and a.status = ?
                     group by a.id
