<template>
	<div class="popup address-province-popup">
				...
				<div class="page">
					<div class="navbar">
						<div class="navbar-inner">
							...
							<div class="center text-gradient" v-text="$t('app.page.address_province')">选择省份</div>
							<div class="right" style="padding-right: 0.2rem;">
								<a @click="next" href="javascript:void(0);" class="text-gradient" v-text="$t('app.address_province.next')">下一步</a>
							</div>
						</div>
					</div>
					<div class="page-content">
						<div class="list-block">
							<ul>
								<li :class="{ active: province.Value === selected_province.Value }" @click="onSelectProvince(province)" v-for="province in address_province_list">
		...
		methods: {
			...
			next: function() {
				...
				if(that.selected_province.Value.trim().length === 0) {
					that.$f7.alert(that.$t('app.address_province.please_select_province'))
				} else {
					// 设置选中省份信息
					that.$store.dispatch('setAddressInfo', {
						...
						that.$f7.showPreloader(that.$t('app.address_province.loading_city_data'))
						axios.get('Member/Common/GetChildDistrictByParentId', {
							...
							if(res.IsSuccess === true) {
								that.$store.dispatch('setAddressCity', {
									city_list: res.Data
								}).then(() => {
									// 如果选中的省份没有城市列表，则直接关闭
									if(that.address_city_list.length === 0) {
										that.onFinishSelect()
									} else {
										that.$f7.popup('.address-city-popup')
									}
								})
							} else {
