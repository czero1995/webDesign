<template>
	<div class="popup address-country-popup">
				...
				<div class="page">
					<div class="navbar">
						<div class="navbar-inner">
							...
							<div class="center text-gradient" v-text="$t('app.page.address_country')">选择国家</div>
							<div class="right" style="padding-right: 0.2rem;">
								<a @click="next" href="javascript:void(0);" class="text-gradient" v-text="$t('app.address_country.next')">下一步</a>
							</div>
						</div>
					</div>
					<div class="page-content">
						<div class="list-block">
							<ul>
								<li :class="{ active: country.Value === selected_country.Value }" @click="onSelectCountry(country)" v-for="country in address_country_list">
...
<script>
	...
	export default {
		...
		computed: {
			...mapState({
				address_country_list: state => state.address_country_list,
				address_province_list: state => state.address_province_list
			})
		},
		mounted() {
			this.$nextTick(_ => {
		...
		methods: {
			...
			next: function() {
				...
				if(that.selected_country.Value.trim().length === 0) {
					that.$f7.alert(that.$t('app.address_country.please_select_country'))
				} else {
					...
					that.$store.dispatch('setAddressInfo', {
						...
						that.$f7.showPreloader(that.$t('app.address_country.loading_province_data'))
						axios.get('Member/Common/GetFirstDistrictByCountryCode', {
							params: {
								countryCode: that.selected_country.Value
							}
						}).then(res => {
							console.log('getAddressProvince', res)
							that.$f7.hidePreloader()
							if(res.IsSuccess === true) {
								that.$store.dispatch('setAddressProvince', {
									province_list: res.Data
								}).then(() => {
									// 如果选中的国家没有省份列表，则直接关闭
									if(that.address_province_list.length === 0) {
										that.onFinishSelect()
									} else {
										that.$f7.popup('.address-province-popup')
									}
								})
							} else {
