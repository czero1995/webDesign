...
<<<<<<< .mine
					...
					that.$store.dispatch('forgetPassword',{
					email:Email,
					id:ID
				}).then(() => {
					setTimeout(function(){
						if(that.ajax_result.isSuccess != null){
							if(that.ajax_result.isSuccess === true){
								console.log('true')
							}else{							
									// 登录失败，弹出提示内容
									that.$f7.alert('发送失败：' + that.ajax_result.errorInfos[0].Message, '億嘉國際', function() {
										console.log('发送邮件失败')
									})
							}
						}
