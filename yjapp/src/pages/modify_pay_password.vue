<template>
	<div data-page="modify-pay-password" class="page modify-pay-password-page">
		<!-- Top Navbar-->
		<div class="navbar">
			<div class="navbar-inner">
				...
				<div class="center"><span class="text-gradient" v-text="$t('app.page.modify_pay_password')">修改支付密码</span></div>
			</div>
		</div>
		<!-- /End of Top Navbar-->
		<div class="page-content">
			<div class="list-block">
				<ul>
					<li class="item-content">
						<input v-show="showOldPassword" type="text" class="input-password" :placeholder="$t('app.modify_pay_password.input_old_password')" v-model="oldpassword" />
						<input v-show="!showOldPassword" type="password" class="input-password" :placeholder="$t('app.modify_pay_password.input_old_password')" v-model="oldpassword" />
					...
					<li class="item-content clearfloat">
						<input v-show="showNewPassword" type="text" class="input-password" :placeholder="$t('app.modify_pay_password.input_new_password')" v-model="newpassword" />
						<input v-show="!showNewPassword" type="password" class="input-password" :placeholder="$t('app.modify_pay_password.input_new_password')" v-model="newpassword" />
			...
			<div @click="onModify" class="modify-insure" v-text="$t('app.modify_pay_password.submit')">确定</div>
		</div>
	</div>
</template>

<style lang="less">
	.modify-pay-password-page {
		.list-block {
			margin: 0;
		}
		.item-content {
			...
			.view-password {
				position: absolute;
				background: url(../../static/icon/login_visible_nor@3x.png) no-repeat;
				width: 0.44rem;
				height: 0.44rem;
				background-size: 100%;
		...
		methods: {
			...
			onModify: function() {
				...
				if(that.oldpassword.trim().length === 0 || that.newpassword.trim().length === 0) {
					that.$f7.alert(that.$t('app.modify_pay_password.password_required'))
				} else {
